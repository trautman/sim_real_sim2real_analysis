import numpy as np
from scipy import stats
from scipy.stats import spearmanr
import matplotlib.pyplot as plt
import yaml
import os
import warnings
import ast

from matplotlib.lines import Line2D
from matplotlib.container import ErrorbarContainer
from matplotlib.legend_handler import HandlerErrorbar

np.seterr(all='ignore')
warnings.simplefilter("ignore", category=RuntimeWarning)


def try_parse_list(val):
    if isinstance(val, str):
        try:
            return ast.literal_eval(val)
        except Exception:
            return val
    else:
        return val


def calling_data_files(file1, file2, file3):
    files = [file1, file2, file3]
    data_list = []
    for f in files:
        with open(f) as fp:
            data_list.append(yaml.safe_load(fp))
    # Initialize lists for keys
    sites, states, names, metrics, means, stds, Ns, bins_ = [[] for _ in range(8)]
    array_keys = {'mean', 'std'}
    keys = ['site', 'state', 'baseline', 'metric', 'mean', 'std', 'N', 'bin']
    containers = [sites, states, names, metrics, means, stds, Ns, bins_]
    for data in data_list:
        for key, container in zip(keys, containers):
            value = np.array(data[key]) if key in array_keys else data[key]
            container.append(value)
    index = min(len(m) for m in means)
    means_true = [m.copy() for m in means]
    stds_true = [s.copy() for s in stds]
    bins_true = [np.array(b) for b in bins_]
    Ns_true = [np.array(n) for n in Ns]
    for i in range(len(means)):
        means[i] = means[i][:index]
        stds[i] = stds[i][:index]
        bins_[i] = bins_[i][:index]
        Ns[i] = Ns[i][:index]
    site_1, site_2, site_3 = sites
    state_1, state_2, state_3 = states
    name_1, name_2, name_3 = names
    metric_1, metric_2, metric_3 = metrics
    m_1, m_2, m_3 = means
    std_1, std_2, std_3 = stds
    b_1, b_2, b_3 = bins_
    N_1, N_2, N_3 = Ns
    m_1_true, m_2_true, m_3_true = means_true
    std_1_true, std_2_true, std_3_true = stds_true
    b_1_true, b_2_true, b_3_true = bins_true
    N_1_true, N_2_true, N_3_true = Ns_true
    return (site_1, state_1, name_1, metric_1, m_1, std_1, b_1, N_1,
            site_2, state_2, name_2, metric_2, m_2, std_2, b_2, N_2,
            site_3, state_3, name_3, metric_3, m_3, std_3, b_3, N_3,
            m_1_true, std_1_true, b_1_true, N_1_true,
            m_2_true, std_2_true, b_2_true, N_2_true,
            m_3_true, std_3_true, b_3_true, N_3_true)


def hedges_g(m_1, m_2, std_1, std_2, N_1, N_2, compare_name):
    s1, s2 = np.power(std_1, 2), np.power(std_2, 2)
    with np.errstate(divide='ignore', invalid='ignore'):
        s = np.sqrt(((N_1 - 1) * s1 + (N_2 - 1) * s2) / (N_1 + N_2 - 2))
    g = (m_1 - m_2) / s
    g = np.around(g, 2)
    J = 1 - (3 / (4 * (N_1 + N_2) - 9))
    g = np.around(J * g, 2)
    se_g = np.around(np.sqrt((N_1 + N_2) / (N_1 * N_2) + (g**2) / (2 * (N_1 + N_2))), 2)
    confidence = 0.95
    z_value = stats.norm.ppf(1 - (1 - confidence) / 2)
    ci_upper = np.around(g + z_value * se_g, 3)
    ci_lower = np.around(g - z_value * se_g, 3)
    return g, se_g, z_value, ci_upper, ci_lower


def lin_regress(name_1, name_2, name_3,
                metric_1, metric_2, metric_3,
                bin_arr_1, bin_arr_2, bin_arr_3,
                mean_arr_1, mean_arr_2, mean_arr_3,
                g_brne_dwb, g_brne_human,
                index_bd, index_bh,
                site_1, site_2, site_3):
    main_data = [(bin_arr_1, mean_arr_1), (bin_arr_2, mean_arr_2), (bin_arr_3, mean_arr_3)]
    slopes, intercepts, r_vals, p_linears = [], [], [], []
    r_spears, p_spears = [], []
    for bins, means in main_data:
        res = stats.linregress(bins, means)
        slopes.append(round(res.slope, 2))
        intercepts.append(round(res.intercept, 3))
        r_vals.append(round(res.rvalue, 2))
        p_linears.append(round(res.pvalue, 3))
        r_spear, p_spear = spearmanr(bins, means)
        r_spears.append(r_spear)
        p_spears.append(p_spear)
    slope_1, slope_2, slope_3 = slopes
    intercept_1, intercept_2, intercept_3 = intercepts
    r_1, r_2, r_3 = r_vals
    p_linear_1, p_linear_2, p_linear_3 = p_linears
    r_spear_1, r_spear_2, r_spear_3 = r_spears
    p_spear_1, p_spear_2, p_spear_3 = p_spears

    g_data = [(bin_arr_1[:index_bd], g_brne_dwb),
              (bin_arr_1[:index_bh], g_brne_human)]
    slopes_g, intercepts_g, r_vals_g, p_linears_g = [], [], [], []
    r_spears_g, p_spears_g = [], []
    for bins_g, g_vals in g_data:
        res_g = stats.linregress(bins_g, g_vals)
        slopes_g.append(round(res_g.slope, 3))
        intercepts_g.append(round(res_g.intercept, 3))
        r_vals_g.append(round(res_g.rvalue, 2))
        p_linears_g.append(round(res_g.pvalue, 3))
        r_g_spear, p_g_spear = spearmanr(bins_g, g_vals)
        r_spears_g.append(r_g_spear)
        p_spears_g.append(p_g_spear)
    slope_g_brne_dwb, slope_g_brne_human = slopes_g
    intercept_g_brne_dwb, intercept_g_brne_human = intercepts_g
    r_g_brne_dwb, r_g_brne_human = r_vals_g
    p_linear_g_brne_dwb, p_linear_g_brne_human = p_linears_g
    r_g_spear_brne_dwb, r_g_spear_brne_human = r_spears_g
    p_g_spear_brne_dwb, p_g_spear_brne_human = p_spears_g

    return (slope_1, slope_2, slope_3,
            slope_g_brne_dwb, slope_g_brne_human,
            intercept_1, intercept_2, intercept_3,
            intercept_g_brne_dwb, intercept_g_brne_human,
            r_1, r_2, r_3,
            r_g_brne_dwb, r_g_brne_human,
            p_linear_1, p_linear_2, p_linear_3,
            p_linear_g_brne_dwb, p_linear_g_brne_human,
            r_spear_1, r_spear_2, r_spear_3,
            p_spear_1, p_spear_2, p_spear_3,
            r_g_spear_brne_dwb, p_g_spear_brne_dwb,
            r_g_spear_brne_human, p_g_spear_brne_human)


def ttest_p_distribution(m_1, m_2, std_1, std_2, N_1, N_2, trials=1000):
    np.random.seed(42)
    p_distributions = [[] for _ in range(min(len(m_1), len(m_2)))]
    for _ in range(trials):
        for bin in range(min(len(m_1), len(m_2))):
            rvs1 = stats.norm.rvs(loc=m_1[bin], scale=std_1[bin], size=N_1[bin])
            rvs2 = stats.norm.rvs(loc=m_2[bin], scale=std_2[bin], size=N_2[bin])
            t_stat, p = stats.ttest_ind(rvs1, rvs2)
            p_distributions[bin].append(p)
    return p_distributions


from matplotlib.container import ErrorbarContainer
from matplotlib.legend_handler import HandlerErrorbar
import numpy as np

def error_and_line_fit_baseline(
    name_1, name_2, name_3,
    site_1, site_2, site_3,
    state_1, state_2, state_3,
    metric_1, metric_2, metric_3,
    b_1, b_2, b_3,
    m_1, m_2, m_3,
    std_1, std_2, std_3,
    s_1, s_2, s_3,
    i_1, i_2, i_3,
    r_1, r_2, r_3,
    p_1, p_2, p_3,
    N_1, N_2, N_3,
    ax,
    r_spear_1, r_spear_2, r_spear_3,
    p_spear_1, p_spear_2, p_spear_3,
    fontsize,
    error_marker_size,
    regression_linewidth,
    error_capsize,
    legend_fontsize,
    correlation_threshold,
    performance_title,
    ci_linewidth,
    legend_location,
    xy_label_size
):
    """
    Plots error bars and (if significant) regression lines for three baselines.
    It uses the provided standard errors (computed as std/sqrt(N)) to show 95% CI,
    and then attempts to plot a regression line if Pearson's p-value is < 0.05.
    A legend is created using only the error bars.
    """
    # COMPUTING STANDARD ERROR (ste) for each baseline
    ste_1 = std_1 / np.sqrt(N_1)
    ste_2 = std_2 / np.sqrt(N_2)
    ste_3 = std_3 / np.sqrt(N_3)

    # Group the provided parameters in lists for iteration.
    sites      = [site_1, site_2, site_3]
    names      = [name_1, name_2, name_3]
    metrics    = [metric_1, metric_2, metric_3]
    b_values   = [b_1, b_2, b_3]
    m_values   = [m_1, m_2, m_3]
    ste_values = [ste_1, ste_2, ste_3]
    colors     = ['blue', 'magenta', 'red']

    # Regression-related parameters
    intercepts    = [i_1, i_2, i_3]
    slopes        = [s_1, s_2, s_3]
    pearson_r     = [r_1, r_2, r_3]
    pearson_p     = [p_1, p_2, p_3]
    spearman_r    = [r_spear_1, r_spear_2, r_spear_3]
    spearman_p    = [p_spear_1, p_spear_2, p_spear_3]

    error_handles = []
    regression_handles = []

    # 1. Plot the error bars for each baseline.
    for i in range(3):
        label_text = f"{names[i]} at {sites[i]} $\mu \pm 95\%$CI"
        error_container = ax.errorbar(
            b_values[i],
            m_values[i],
            yerr=1.96 * ste_values[i],
            fmt='o',
            markersize=error_marker_size,
            linewidth=ci_linewidth,
            capsize=error_capsize,
            color=colors[i],
            linestyle='',
            label=label_text
        )
        error_handles.append(error_container)

        # 2. Plot a regression line if the correlation is significant.
        if pearson_p[i] < 0.05:
            label = (f"{names[i]} nominal=${intercepts[i]:.1f}m$, "
                     f"$\\Delta_{{perf}}={slopes[i]:.1f}\\Delta m/\\Delta\\rho$, "
                     f"$r={pearson_r[i]:.1f}$")
            reg_line, = ax.plot(
                b_values[i],
                intercepts[i] + slopes[i] * np.array(b_values[i]),
                '--',
                linewidth=regression_linewidth,
                label=label,
                color=colors[i]
            )
        elif spearman_p[i] < 0.05:
            label = f"{names[i]} monotonic correlation $r_s={spearman_r[i]:.2f}$"
            reg_line, = ax.plot(
                [], [], 'r--',
                linewidth=regression_linewidth,
                label=label,
                color=colors[i]
            )
        else:
            label = f"{names[i]} no correlation"
            reg_line, = ax.plot(
                [], [], 'r--',
                linewidth=regression_linewidth,
                label=label,
                color=colors[i]
            )
        regression_handles.append(reg_line)
        # Print regression parameters for debugging.
        print(f"{sites[i]} {names[i]} {metrics[i]} "
              f"p_l = {pearson_p[i]:.1f}, "
              f"beta_0={intercepts[i]:.1f}, "
              f"d_perf/d_rho= {slopes[i]:.1f}, "
              f"r_l={pearson_r[i]:.1f}, "
              f"p_m = {spearman_p[i]:.1f}, "
              f"r_m={spearman_r[i]:.1f}")

    # Use only the error bar containers for the legend.
    handles = error_handles
    labels = [h.get_label() for h in error_handles]

    ax.legend(
        handles=handles,
        labels=labels,
        fontsize=legend_fontsize,
        handler_map={ErrorbarContainer: HandlerErrorbar()},
        loc=legend_location
    )

    # Set plot title and axis labels.
    ax.set_title(f"{performance_title}", fontsize=fontsize)
    ax.set_xlabel("Mean Crowd Density (people$/m^2$)", fontsize=fontsize)
    ax.set_ylabel(metric_1, fontsize=fontsize)
    ax.tick_params(axis='both', labelsize=xy_label_size)




def error_and_line_fit_g(
    name_1, name_2, name_3,
    site_1, site_2, site_3,
    metric_1, metric_2, metric_3,
    b_1, b_2, b_3,
    g_brne_dwb, g_brne_human,
    se_g_brne_dwb, se_g_brne_human,
    s_g_brne_dwb, s_g_brne_human,
    i_g_brne_dwb, i_g_brne_human,
    r_g_brne_dwb, r_g_brne_human,
    p_lin_g_brne_dwb, p_lin_g_brne_human,
    ax, index_bd, index_bh,
    r_g_spear_brne_dwb, p_g_spear_brne_dwb,
    r_g_spear_brne_human, p_g_spear_brne_human,
    fontsize,
    error_marker_size,
    regression_linewidth,
    error_capsize,
    legend_fontsize,
    lim_x,
    if_g_line_fit,
    correlation_threshold,
    performance_gap_title,
    ci_threshold, ci_linewidth, legend_location, xy_label_size
):
    """
    Plots g-values for BRNE vs two baselines (e.g., DWB and HUMAN) with optional line fits,
    filtering out points with large 95% CI (if ci_threshold is set).
    Ensures the legend shows both the marker and error-bar caps.
    """
    # Helper function for filtering data by CI threshold
    def filter_ci(x_vals, y_vals, se_vals, idx):
        """
        x_vals: e.g., b_1
        y_vals: e.g., g_brne_dwb
        se_vals: e.g., se_g_brne_dwb
        idx: integer slice limit (e.g., index_bd)
        Returns (x_filtered, y_filtered, se_filtered).
        """
        x_slice = x_vals[:idx]
        y_slice = y_vals[:idx]
        se_slice = se_vals[:idx]
        if ci_threshold is not None:
            ci_95 = 1.96 * se_slice
            mask = ci_95 <= ci_threshold
            return x_slice[mask], y_slice[mask], se_slice[mask]
        else:
            return x_slice, y_slice, se_slice

    # 1) Filter / slice each dataset
    b_1_bd, g_dwb, se_dwb = filter_ci(b_1, g_brne_dwb, se_g_brne_dwb, index_bd)
    b_3_bh, g_human, se_human = filter_ci(b_3, g_brne_human, se_g_brne_human, index_bh)

    # 2) Plot error bars using ErrorbarContainer
    error_container1 = ax.errorbar(
        b_1_bd, g_dwb, yerr=1.96 * se_dwb,
        fmt='o', markersize=error_marker_size,
        linewidth=ci_linewidth,
        capsize=error_capsize,
        linestyle='-',
        color='magenta'
    )
    error_container2 = ax.errorbar(
        b_3_bh, g_human, yerr=1.96 * se_human,
        fmt='o', markersize=error_marker_size,
        linewidth=ci_linewidth,
        capsize=error_capsize,
        linestyle='-',
        color='red'
    )

    error_handles = [error_container1, error_container2]

    # 3) Optionally add line fits + threshold lines if if_g_line_fit == 'True'
    regression_handles = []
    threshold_handles = []

    if if_g_line_fit == 'True':

        # --- Baseline1 vs Baseline2 ---
        if p_lin_g_brne_dwb < 0.05:
            label_dwb = (f"{site_1}v{site_2} nominal={round(i_g_brne_dwb,2)}, "
                         f"$\\Delta_{{gap}}={round(s_g_brne_dwb,2)}\\Delta g/\\Delta\\rho$, "
                         f"$r={r_g_brne_dwb}$")
            line_brne_dwb, = ax.plot(
                b_1_bd,
                i_g_brne_dwb + s_g_brne_dwb * b_1_bd,
                'k--',
                linewidth=regression_linewidth,
                label=label_dwb,
                color='magenta'
            )
        elif p_g_spear_brne_dwb < 0.05:
            line_brne_dwb, = ax.plot([], [], 'k--',
                label=f"$g$ {site_1}v{site_2} monotonic correlation $r_s={round(r_g_spear_brne_dwb,2)}$",
                linewidth=regression_linewidth,
                color='magenta'
            )
        else:
            line_brne_dwb, = ax.plot([], [], 'k--',
                label=f"$g$ {site_1}v{site_2} no correlation",
                linewidth=regression_linewidth,
                color='magenta'
            )
        regression_handles.append(line_brne_dwb)
        print(f"GAP {site_1}v{site_2} {metric_1} {site_1} p_l^g={p_lin_g_brne_dwb:.1f}, "
              f"beta_0^g={i_g_brne_dwb:.1f}, d_gap/d_rho={s_g_brne_dwb:.1f}, "
              f"r_l^g={r_g_brne_dwb:.1f}, p_m^g={p_g_spear_brne_dwb:.1f}, "
              f"r_m^g={r_g_spear_brne_dwb:.1f}")

        # --- Baseline1 vs Baseline3 ---
        if p_lin_g_brne_human < 0.05:
            label_human = (f"{site_1}v{site_3} nominal={round(i_g_brne_human,2)}, "
                           f"$\\Delta_{{gap}}={round(s_g_brne_human,2)}\\Delta g/\\Delta\\rho$, "
                           f"$r={r_g_brne_human}$")
            line_brne_human, = ax.plot(
                b_3_bh,
                i_g_brne_human + s_g_brne_human * b_3_bh,
                'k--',
                linewidth=regression_linewidth,
                label=label_human,
                color='red'
            )
        elif p_g_spear_brne_human < 0.05:
            line_brne_human, = ax.plot([], [], 'k--',
                label=f"$g$ {site_1}v{site_3} monotonic correlation $r_s={round(r_g_spear_brne_human,2)}$",
                linewidth=regression_linewidth,
                color='red'
            )
        else:
            line_brne_human, = ax.plot([], [], 'k--',
                label=f"$g$ {site_1}v{site_3} no correlation",
                linewidth=regression_linewidth,
                color='red'
            )
        regression_handles.append(line_brne_human)
        print(f"GAP {site_1}v{site_3} {metric_1} {site_1} p_l^g={p_lin_g_brne_human:.1f}, "
              f"beta_0^g={i_g_brne_human:.1f}, d_gap/d_rho={s_g_brne_human:.1f}, "
              f"r_l^g={r_g_brne_human:.1f}, p_m^g={p_g_spear_brne_human:.1f}, "
              f"r_m^g={r_g_spear_brne_human:.1f}")

        # --- Threshold lines ---
        thresholdupper, = ax.plot(
            [0, lim_x], [0.5, 0.5], color='k', linestyle='-', lw=3.0,
            label='$g>0.5$ BRNE outperforms baseline'
        )
        thresholdlower, = ax.plot(
            [0, lim_x], [-0.5, -0.5], color='k', linestyle='-', lw=3.0,
            label='$g<-0.5$ baseline outperforms BRNE'
        )
        threshold_handles.extend([thresholdupper, thresholdlower])

        # Combine error bars and threshold lines for legend
        handles = error_handles + threshold_handles
        bin_label = f'Arcadev{{}} performance gap $\mu \pm 95\%$CI'
        labels = [
            bin_label.format(site_2),
            bin_label.format(site_3)
        ] + [thresholdupper.get_label(), thresholdlower.get_label()]

    else:  # If no regression is conducted
        thresholdupper, = ax.plot(
            [0, lim_x], [0.5, 0.5],
            color='k', linestyle='-', linewidth=regression_linewidth,
            label='$g>0.5$ BRNE outperforms baseline'
        )
        thresholdlower, = ax.plot(
            [0, lim_x], [-0.5, -0.5],
            color='k', linestyle='-', linewidth=regression_linewidth,
            label='$g<-0.5$ baseline outperforms BRNE'
        )
        threshold_handles = [thresholdupper, thresholdlower]
        bin_label = 'BRNEv{} Performance Gap'
        handles = error_handles + threshold_handles
        labels = [
            bin_label.format(name_2),
            bin_label.format(name_3),
            thresholdupper.get_label(),
            thresholdlower.get_label()
        ]

    ax.legend(
        handles=handles,
        labels=labels,
        fontsize=legend_fontsize,
        handler_map={ErrorbarContainer: HandlerErrorbar()},
        loc=legend_location
    )

    ax.set_title(f"{performance_gap_title}", fontsize=fontsize)
    ax.set_xlabel("Mean Crowd Density (people$/m^2$)", fontsize=fontsize)
    ax.set_ylabel(f"{metric_1} Site Performance Gap", fontsize=fontsize)
    ax.tick_params(axis='both', labelsize=xy_label_size)

