...
def hedges_g(m_1, m_2, std_1, std_2, N_1, N_2, compare_name):
    ...
    # here we now return (ci_lower, ci_upper) instead of (ci_upper, ci_upper)
...
def error_and_line_fit_baseline(
    ...
    for i in range(4):
        if i == 0:
            label_text = f"Real pedestrians"
        else:
            label_text = f"{states[i]} pedestrians"
    ...
    ax.set_title(f"{performance_title} at {site_1}", fontsize=fontsize)
    if "max" in iv:
...
def error_and_line_fit_g(
    ...
    ax.set_title(f"{performance_gap_title} at {site_1}", fontsize=fontsize)
    if "max" in iv:
        ax.set_xlabel("Max Crowd Density (people$/m^2$)", fontsize=fontsize)
    else:
        ax.set_xlabel("Mean Crowd Density (people$/m^2$)", fontsize=fontsize)
