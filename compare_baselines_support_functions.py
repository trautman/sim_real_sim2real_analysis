...
def error_and_line_fit_baseline(
    ...
    for i in range(4):
        label_text = f"{names[i]} performance $\mu \pm 95\%$CI"
        ...
        print(f"PERF {metric_1} {site_1} {names[i]} "
    ...
    ax.set_title(f"{performance_title}, {site_1}", fontsize=fontsize)
    if "max" in iv:
...
def error_and_line_fit_g(
    ...
    if if_g_line_fit == 'True':
        ...
        print(f"GAP BRNEvDWB {metric_1} {site_1} "
        ...
        # PRINT REGRESSION PARAMETERS BRNEVHUMAN
        print(f"GAP BRNEvHUMAN {metric_1} {site_1} "
        ...
        print(f"GAP BRNEvTELEOP {metric_1} {site_1} "
        ...
        bin_label = f'BRNEv{{}} performance gap $\mu \pm 95\%$CI' #FOR CI HANDLES
        labels = [
            bin_label.format(name_2),
            bin_label.format(name_3),
            bin_label.format(name_4)] \
    ...
    if "max" in iv:
        ...
        ax.set_xlabel("Mean Crowd Density (people$/m^2$)", fontsize=fontsize)
    ax.set_ylabel(f"{metric_1} Performance Gap", fontsize=fontsize)
    ax.tick_params(axis='both', labelsize=xy_label_size)
